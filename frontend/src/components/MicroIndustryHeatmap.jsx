...
const MicroIndustryHeatmap = ({ onScanNavigate, theme }) => {
    ...
    useEffect(() => {
        fetch('https://algo-scanner-lnck.onrender.com/api/micro-industry-heatmap')
            .then(res => res.json())
            .then(res => {
    ...
    const selectPerformers = () => {
        const performers = displayData.filter(item => (Number(item?.avg_rs) || 0) > 0).map(item => item?.basic_industry).filter(Boolean);
        setSelected(performers);
    };

    const selectUnderperformers = () => {
        const underperformers = displayData.filter(item => (Number(item?.avg_rs) || 0) < 0).map(item => item?.basic_industry).filter(Boolean);
        setSelected(underperformers);
    };

    const clearAll = () => setSelected([]);

    if (loading) return <div style={{ padding: '40px', textAlign: 'center', fontSize: '15px', fontWeight: '600', color: t.textMuted }}>Analyzing Micro Industry Breadth Data...</div>;
    ...
    const displayData = (Array.isArray(data) ? data : [])
        ...
        .filter(item => {
            ...
            return (
                item?.basic_industry?.toLowerCase().includes(term) ||
            ...
            <div style={{ position: 'sticky', top: 0, zIndex: 9999, backgroundColor: t.bgPanel, boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', borderRadius: '0 0 8px 8px', borderBottom: `1px solid ${t.border}` }}>
                <div style={{ padding: '20px 24px 15px 24px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div>
                            <h2 style={{ fontSize: '20px', fontWeight: '800', margin: 0 }}>ALL MICRO INDUSTRIES BREADTH</h2>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                            <input
                                type="text"
                                placeholder="🔍 Filter Micro Industries..."
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                                style={{ padding: '10px 15px', borderRadius: '6px', border: `1px solid ${t.border}`, fontSize: '13px', width: '240px', outline: 'none', backgroundColor: t.inputBg, color: t.textMain }}
                            />
                            <button
...
export default MicroIndustryHeatmap;
